...
//testing malloc and free functionality
Test(sfmm_basecode_suite, malloc_free_test, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
...
//reallocing size of 0
Test(sfmm_basecode_suite, sf_realloc_free, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void *a = sf_malloc(16);
	sf_malloc(200);
	a = sf_realloc(a, 0);
	...
	assert_free_block_count(0, 2);
	assert_free_block_count(32, 1);
	assert_free_block_count(1728, 1);
	assert_free_list_size(0, 1);
	assert_free_list_size(7, 1);
...
//test regualr malloc
Test(sfmm_basecode_suite, sf_realloc_test, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void *a = sf_malloc(16);
	sf_malloc(200);
	a = sf_realloc(a, 100);
...
//check if the payload address is aligned
Test(sfmm_basecode_suite, sf_memalign_check_1, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void *a = sf_malloc(16);
	sf_malloc(200);
	void *b = sf_memalign(32, 256);

	cr_assert_not_null(a, "a is  NULL!");
	cr_assert_not_null(b, "a is  NULL!");

	cr_assert(((size_t) b % 256) == 0, "b is not aligned");

	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}

//check if the payload address is aligned
Test(sfmm_basecode_suite, sf_memalign_check_2, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	void *a = sf_malloc(64);
	sf_malloc(100);
	void *b = sf_memalign(64, 64);

	cr_assert_not_null(a, "a is  NULL!");
	cr_assert_not_null(b, "a is  NULL!");

	cr_assert(((size_t) b % 64) == 0, "b is not aligned");
