...
/*
 * The heap is designed to keep the payload area of each block aligned to a four-row (32-byte)
 ...
 * heap, subject to the condition that its payload area is eight-row aligned.
 */
 ...
 * and so on.  This continues up to the list at index NUM_FREE_LISTS-2 (i.e. 6), which
 * contains blocks whose size is greater than 21M.  The list at index NUM_FREE_LISTS-1
 * (i.e. 7) is only used to contain the "wilderness block", if there is one.
 *
